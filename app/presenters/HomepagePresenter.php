<?php

namespace App\Presenters;

use App\Model\Entities\Article;
use App\Model\Entities\User;
use Tracy\Debugger;

class HomepagePresenter extends BasePresenter
{

    /**
     * @inject
     * @var \Kdyby\Doctrine\EntityManager
     */
    public $EntityManager;

    public function handleChangeArticle($id)
    {
        if ($this->isAjax()) {
            $this->redrawControl('clanek');                    
            $this->redrawControl('wrapper');                    
            $this->template->clanek = $this->EntityManager->getRepository(Article::getClassName())->findOneBy(array("id"=>$id));
        } else {
            $this->redirect("Homepage:default");
        }
    }

    public function renderDefault()
    {
        $dao = $this->EntityManager->getRepository(Article::getClassName());
        //Debugger::dump($dao);

//        dump($dao->findAll());
//        exit();
        $this->template->articles = $dao->findAll();        
        $uzivatel = $this->EntityManager->getRepository(User::getClassName());
//        dump($uzivatel->findOneBy(array("id"=>1)));
//        exit();
        $this->template->user = $uzivatel->findOneBy(array("id"=>1)); 
        //dump($this->template->user-photo);
        //exit();
        //$this->template->photo = Image::fromString(base64_decode(""));
        //$this->template->user->photo = stream_get_contents($this->template->user->getPhoto()); // je nutné provést kvůli zobrazení obrázku
    }

    public function renderArticles()
    {
        $dao = $this->EntityManager->getRepository(Article::getClassName());
//        dump($dao->findAll());
//        exit();
        $this->template->articles = $dao->findAll();        
        $cl = "normal";
        //$this->EntityManager->getRepository(Article::getClassName())->findOneBy(array("id"=>1));
        if (!isset($this->template->clanek))
            $this->template->clanek = $this->EntityManager->getRepository(Article::getClassName())->findOneBy(array("id"=>1));
    }
    
    
    public function actionDetail($id){
        if($this->isAjax()){           
            $this->redrawControl('dialog'); 
            $this->setView('detail');
        }
    }

    public function renderDetail($id){
        $this->template->data = $this->EntityManager->getRepository(Article::getClassName())->findOneBy(array("id"=>$id));
    }

    public function renderDetailAjax($id){
        if(!$this->isAjax()){
            throw new \Nette\Application\BadRequestException();
        }
        $this->template->data = $this->EntityManager->getRepository(Article::getClassName())->findOneBy(array("id"=>$id));
    }
   
}
